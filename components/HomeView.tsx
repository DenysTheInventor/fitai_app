...
import type { DailyLog, View, CheckIn, HabitLog, ReadingHabitLog, GenericHabitLog, CustomHabit } from '../types';
import { HabitType } from '../types';
import { DumbbellIcon, ForkKnifeIcon, MoonIcon, LanguageIcon, BookOpenIcon, PencilSquareIcon, SparklesIcon } from '../constants';
...
const HomeView: React.FC<HomeViewProps> = ({ todayLog, allLogs, setView, setSelectedDate, checkIns, customHabits, setHabitToLog }) => {
        ...
        <div className="space-y-6">
            ...
            <div className="bg-surface dark:bg-dark-surface shadow-sm dark:shadow-none p-4 rounded-lg">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={() => setHabitToLog({type: HabitType.English})} className="text-sm text-primary dark:text-dark-primary font-semibold">Log English</button>
                </div>
                {englishLog ? (
                    <div className="bg-card dark:bg-dark-card p-3 rounded-md">
            ...
            <div className="bg-surface dark:bg-dark-surface shadow-sm dark:shadow-none p-4 rounded-lg">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={() => setHabitToLog({type: HabitType.Reading})} className="text-sm text-primary dark:text-dark-primary font-semibold">Log Reading</button>
                </div>
                {readingLog ? (
            ...
            <div className="bg-surface dark:bg-dark-surface shadow-sm dark:shadow-none p-4 rounded-lg">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={() => setHabitToLog({type: HabitType.Blogging})} className="text-sm text-primary dark:text-dark-primary font-semibold">Log Blogging</button>
                </div>
                {bloggingLog ? (
                    <div className="bg-card dark:bg-dark-card p-3 rounded-md">
                        <div className="flex items-center gap-3">
            ...
            {customHabits.map(habit => {
                const log = todayHabits.find(h => h.type === HabitType.Custom && (h as GenericHabitLog).customHabitId === habit.id) as GenericHabitLog | undefined;
                return (
                    <div key={habit.id} className="bg-surface dark:bg-dark-surface shadow-sm dark:shadow-none p-4 rounded-lg">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="font-semibold text-lg text-text-base dark:text-dark-text-base">{habit.name}</h3>
                            <button onClick={() => setHabitToLog({type: HabitType.Custom, customHabitId: habit.id, customHabitName: habit.name})} className="text-sm text-primary dark:text-dark-primary font-semibold">Log</button>
                        </div>
                        {log ? (
                            <div className="bg-card dark:bg-dark-card p-3 rounded-md">
                                <div className="flex items-center gap-3">
                                    <SparklesIcon className="w-5 h-5 text-secondary dark:text-dark-secondary" />
                                    <span className="text-text-base dark:text-dark-text-base text-sm font-medium">{log.durationMinutes} minutes</span>
                                </div>
                                {log.notes && <p className="text-xs text-text-secondary dark:text-dark-text-secondary mt-2 pl-8">{log.notes}</p>}
                            </div>
                        ) : (
                            <p className="text-center text-text-secondary dark:text-dark-text-secondary py-4 text-sm">Not logged for today.</p>
                        )}
