...
import type { DailyLog, View, CustomHabit } from '../types';
...
const ClassicHistoryView: React.FC<Omit<HistoryViewProps, 'allLogs' | 'customHabits'>> = ({ logs, onUpdateLog, setView, setSelectedActivityId }) => {
...
const HistoryView: React.FC<HistoryViewProps> = (props) => {
    ...
    return (
        <div className="space-y-4">
            <div className="flex gap-2 bg-card dark:bg-dark-card p-1 rounded-md">
                <button onClick={() => setActiveTab('history')} className={`w-full p-2 rounded text-sm font-semibold ${activeTab === 'history' ? 'bg-secondary dark:bg-dark-secondary text-white' : 'text-text-secondary dark:text-dark-text-secondary'}`}>History</button>
                <button onClick={() => setActiveTab('overview')} className={`w-full p-2 rounded text-sm font-semibold ${activeTab === 'overview' ? 'bg-secondary dark:bg-dark-secondary text-white' : 'text-text-secondary dark:text-dark-text-secondary'}`}>Overview</button>
            </div>

            {activeTab === 'history' ? (
                <>
                    <ClassicHistoryView {...props} />
                    ...
                    <FilterModal isOpen={isFilterModalOpen} onClose={() => setIsFilterModalOpen(false)} filters={props.filters} onApply={props.setFilters} />
                </>
            ) : (
                <HabitsOverview logs={props.allLogs} customHabits={props.customHabits} />
            )}
        </div>
    );
};
