...
const LogHabitModal: React.FC<LogHabitModalProps> = ({ habitToLog, onClose, onSave, books, allLogs }) => {
    ...
    useEffect(() => {
        if (habitToLog.type === HabitType.Reading && unfinishedBooks.length > 0) {
            setSelectedBookId(unfinishedBooks[0].id);
        }
    }, [habitToLog.type, unfinishedBooks]);
    ...
    const handleSubmit = () => {
        let newLog: HabitLog;
        if (habitToLog.type === HabitType.Reading) {
            if (!selectedBookId) {
        ...
        } else if (habitToLog.type === HabitType.Custom) {
            newLog = { 
                id: new Date().toISOString(), 
                type: HabitType.Custom, 
                durationMinutes: duration, 
                notes: notes,
                customHabitId: habitToLog.customHabitId
            };
        } else {
            newLog = { id: new Date().toISOString(), type: habitToLog.type, durationMinutes: duration, notes: notes } as GenericHabitLog;
        }
        onSave(newLog);
        onClose();
    };
    ...
    return (
         <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-surface dark:bg-dark-surface rounded-lg p-6 w-full max-w-md shadow-xl overflow-y-auto max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
                <h2 className="text-xl font-bold mb-4 text-text-base dark:text-dark-text-base">Log {habitToLog.customHabitName || habitToLog.type}</h2>
                <div className="space-y-4">
                    {habitToLog.type === HabitType.Reading && (
                        <div>
                    ...
                    {(habitToLog.type === HabitType.English || habitToLog.type === HabitType.Blogging || habitToLog.type === HabitType.Custom) && (
                        <div>
