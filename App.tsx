...
import type { DailyLog, View, CustomExercise, UserSettings, AppData, CheckIn, ExerciseSet, OutdoorRunActivity, HabitLog, Book, ReadingHabitLog, GenericHabitLog, CustomHabit } from './types';
...
function App() {
  ...
  const [customHabits, setCustomHabits] = useLocalStorage<CustomHabit[]>('fitai-custom-habits', []);
  const [habitToLog, setHabitToLog] = useState<HabitToLogType | null>(null);

  useEffect(() => {
    const needsMigration = logs.some(log => !log.hasOwnProperty('habits'));
    if (needsMigration) {
      console.log("Migrating log data to new version...");
      const migratedLogs = logs.map(log => ({ ...log, habits: log.habits || [] }));
  ...
  const renderView = () => {
    ...
    const appData: AppData = { logs, customExercises, userSettings, checkIns, exerciseSets, books, customHabits };

    switch (view) {
      case 'home':
        return <HomeView todayLog={getLogForDate(today)} allLogs={logs} setView={setView} setSelectedDate={setSelectedDate} checkIns={checkIns} customHabits={customHabits} setHabitToLog={setHabitToLog} />;
      case 'calendar':
        return <CalendarView logs={logs} checkIns={checkIns} setSelectedDate={setSelectedDate} setView={setView} setSelectedCheckInId={setSelectedCheckInId} customHabits={customHabits} setHabitToLog={setHabitToLog} />;
      case 'routine':
      ...
      case 'history':
        return <HistoryView logs={filteredLogs} allLogs={logs} filters={historyFilters} setFilters={setHistoryFilters} onUpdateLog={updateLogForDate} setView={setView} setSelectedActivityId={setSelectedActivityId} customHabits={customHabits} />;
      case 'exercises':
      ...
      case 'analysis':
        return <AnalysisDashboard allLogs={logs} userSettings={userSettings} checkIns={checkIns} customHabits={customHabits} />;
      case 'settings-hub':
      ...
      case 'habits-library':
        return <HabitsLibraryView habits={customHabits} setHabits={setCustomHabits} />;
  ...
  const getHeaderText = () => {
    switch(view) {
        ...
        case 'habits-library': return 'Manage Habits';
