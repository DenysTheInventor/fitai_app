...
import type { DailyLog, UserSettings, CheckIn, ReadingHabitLog, GenericHabitLog, CustomHabit } from "../types";
import { HabitType } from "../types";

const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

function formatDataForPrompt(logs: DailyLog[], settings: UserSettings, checkIns: CheckIn[], customHabits: CustomHabit[]): string {
    let formattedString = "Here is the user's profile and their data:\n\n";
    const customHabitMap = new Map(customHabits.map(h => [h.id, h.name]));
                ...
                log.habits.forEach(habit => {
                    if (habit.type === HabitType.Reading) {
                        const readingHabit = habit as ReadingHabitLog;
                         ...
                         const habitName = genericHabit.type === HabitType.Custom
                            ? customHabitMap.get(genericHabit.customHabitId || '') || 'Custom Habit'
                            : genericHabit.type;

                         let habitString = `- ${habitName}: ${genericHabit.durationMinutes} minutes`;
...
export const getAiAnalysis = async (logs: DailyLog[], settings: UserSettings, checkIns: CheckIn[], customHabits: CustomHabit[], periodDescription: string): Promise<string> => {
    const dataPrompt = formatDataForPrompt(logs, settings, checkIns, customHabits);
    const fullPrompt = `
        ...
        **Анализ привычек**
        *   Проанализируй последовательность в отслеживаемых привычках (чтение, английский, блоггинг и любые пользовательские привычки).
        *   Дай обратную связь о том, как эти привычки могут влиять на общие цели пользователя.
